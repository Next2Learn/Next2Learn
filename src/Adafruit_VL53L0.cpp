...
boolean Adafruit_VL53L0X::begin(int i2c_addr) {
  //int32_t status_int;
  //int32_t init_done = 0;
Serial.println(i2c_addr);

  digitalWrite(debuggLED2, HIGH);
  // Initialize Comms
  ...
  Serial.println(Status);
  Serial.println("ERROR_None");
  Serial.println(VL53L0X_ERROR_NONE);
...
*/
  if(Status == VL53L0X_ERROR_NONE)  {
    ...
    Serial.println("Status okay begin(int)");
  return true;
}
...
boolean Adafruit_VL53L0X::begin(void) {
  //int32_t status_int;
  //int32_t init_done = 0;
  Serial.println(VL53L0X_I2C_ADDR);
  ...
  Serial.println("Status okay begin()");
...
void Adafruit_VL53L0X::print_range_status(VL53L0X_RangingMeasurementData_t* pRangingMeasurementData){
     ...
     * New Range Status: data is valid when pRangingMeasurementData->RangeStatus = 0
     if RangeStatus==0 then GPIO=HIGH
     */
